...
doing this manually is cumbersome, and, luckily, we can the process easier by implementing it in our smart contracts.
...
1. It makes pool addresses predictable and allows to calculate them without making calls to the registry. This makes
pools easily discoverable.

Let's build Factory contract! But before this, we need to learn a new mechanism.
...
`token0` and `token` are the addresses of pool tokens, and `tickSpacing` is something we haven't learned yet. Let's fix
that!
...
target one, which consumes gas. To make this loop more gas-efficient, Uniswap pools have `tickSpacing` setting which acts
as a scaling factor: `1` means every tick is counted, `10` means only every 10th tick is counted, `60` means only every
60th tick is counter, and so on.

[TODO: illustrate]

However, the higher the tick spacing the lower the precision. Low volatility pairs (e.g. stablecoin pairs) need higher
...
> And, yes, there can pools with the same tokens but different tick spacing.
...
> From now on, we'll assume 60 tick spacing for all our pools.
...
> We could've made them constants, but we'll need to have it as a mapping for a later milestone.
...
(tokenX, tokenY) = tokenX < tokenY
    ...
    : (tokenY, tokenX);
```
From now on, we'll also expect pool token addresses to be sorted, i.e. `token0` goes before `token1` when sorted. We need
this for consistency, to make salt consistent.
...
This piece looks weird because `parameters` is not used. Uniswap uses *Inversion of Control* to pass parameters to a
pool during deployment. Let's look at updated Pool contract constructor:
```solidity
...
Aha! Pool expects its deployer to implement `IUniswapV3PoolDeployer` interface (which only defines `parameters` getter)
and calls it in the constructor during deployment to get the parameters. This is what the flow looks like:
1. `Factory`: defines `parameters` state variable (implements `IUniswapV3PoolDeployer`) and sets them before
deploying a pool.
...
returned.
1. `Factory`: calls `delete parameters;` to clean up the slot of `parameters` state variable and cut gas consumption.
This is a temporary state variable that has a value only during a call to `createPool()`.

After a pool is created, we keep in `pools` mapping (so it can be found by its tokens) and emit an event:
```solidity
...
The function needs to know pool parameters (this they're used to build a salt) and Factory contract address. It expects
the tokens to be sorted, which we discussed above.
...
1. salt was discussed earlier–it's away to identify a unique Pool contract;
1. hashed contract code is needed to protect from collisions: different contract can have the same salt, but their core
will be different.
...
making any external calls and without querying the factory. Easy-peasy!
